...
CONFIG_PATH="/N/project/solsex/_sapp/draft_assembly/pool_config.txt"
CMD_PATH="/N/soft/rhel7/masurca/3.2.8/bin/masurca"
set -o pipefail
...
#}
export PATH="/geode2/soft/hps/rhel7/masurca/3.2.8/bin/../CA/Linux-amd64/bin:/geode2/soft/hps/rhel7/masurca/3.2.8/bin:$PATH"
save PATH
export PERL5LIB=/geode2/soft/hps/rhel7/masurca/3.2.8/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=32
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'pe 350 125' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'pe' <(exec expand_fastq '/N/project/solsex/_sapp/reads/illumina/illum_all_R1.fastq.gz' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/N/project/solsex/_sapp/reads/illumina/illum_all_R2.fastq.gz' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
...
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>30000000000)printf "%.0f",s;else print "30000000000";}'`
save JF_SIZE
perl -e '{if(int('$JF_SIZE')>30000000000){print "WARNING: JF_SIZE set too low, increasing JF_SIZE to at least '$JF_SIZE', this automatic increase may be not enough!\n"}}'
...
quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/geode2/soft/hps/rhel7/masurca/3.2.8/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 16 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor.tmp --verbose 1>quorum.err 2>&1 && mv pe.cor.tmp.fa pe.cor.fa || fail Error correction of PE reads failed. Check pe.cor.log.
...
/geode2/soft/hps/rhel7/masurca/3.2.8/bin/mega_reads_assemble_cluster.sh  -Pb 300000000 -q all.q -G 0 -C 25 -t 16 -e $ESTIMATED_GENOME_SIZE -m work1 -p /N/project/solsex/_sapp/reads/pac/pac_all.fasta -a /geode2/soft/hps/rhel7/masurca/3.2.8/bin/../CA8/Linux-amd64/bin -o "   cgwErrorRate=0.15" -B 17 -D 0.029
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 8000000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'pe.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 16 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
